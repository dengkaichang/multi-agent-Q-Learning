function [ Rayleigh ] = Shadowing( SamplingTime )
if SamplingTime>10^-3;
    Nt=round(SamplingTime/10^-3);
    SamplingTime=10^-3;
else
    Nt=1;
end
Rayleigh(Nt)=1;
for ix=1:Nt;
sampleTime = SamplingTime; % Sample time (s)
maxDopplerShift = 950*30/500; % Maximum Doppler shift of diffuse components (Hz)
...
bitsPerFrame = int64(sampleTime*15*10^6);
...
% values over time
Rayleigh(ix)=mean(abs(filter(rayChanObj,modSignal)));
end
Rayleigh=mean(Rayleigh);
end
