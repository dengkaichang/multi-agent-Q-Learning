...
function ObjValObs = FindObstacle(map,old,new)
    Penalty = 100;
    TempObjValObs = 0;
...
%     x2 = new(1); y2 = new(2);
    ...
    if (x1>0 && x2>0 && y1>0 && y2>0)    
        ...
        % Y ekseni üzerindeki noktalar arasý engeller aranýyor
        if(x1==x2 && y1~=y2) 
            y_ekseni = 1;
            if(y1<y2)
                ilk=y1;son=y2;
        ...
        % X ekseni üzerindeki noktalar arasý engeller aranýyor
        if(y1==y2 && x1~=x2) 
             x_ekseni = 1;
            if(x1<x2)
                ilk=x1;son=x2;
