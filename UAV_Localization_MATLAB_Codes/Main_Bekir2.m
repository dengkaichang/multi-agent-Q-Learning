...
while (Signaling<1)
    ...
    dist_1        = stParameters.LocUE-stParameters.LocUAV_1;
...
%         stParameters.Learning = (0.75);
    ...
    location_1=stParameters.LocUAV_1;
    stParameters=Q_learning(Prec(1)-sum(Prec2)/3,stParameters,Prec(1));
    stParameters=Q_learning_1(Prec_1(1)-sum(Prec3)/3,stParameters,Prec_1(1));
    oldLoc=stParameters.LocUAV;
    oldLoc_1=stParameters.LocUAV_1;
    stParameters.LocUAV = Q_action(stParameters.LocUAV,stParameters.Action,movetime);
    stParameters.LocUAV_1 = Q_action(stParameters.LocUAV_1,stParameters.Action,movetime);
    newLoc=stParameters.LocUAV;
    newLoc_1=stParameters.LocUAV_1;
    ...
    while (err_1>0)
        action = ceil(8*rand(1));
        stParameters.LocUAV_1 = Q_action(oldLoc_1,action,movetime);
        newLoc_1=stParameters.LocUAV_1;
        ObjValObs_1 = feval(ObjObstacle_1,map,oldLoc_1,newLoc_1);
        if (ObjValObs_1 == 100)
            obstacles_1 = [obstacles_1;newLoc_1];
...
%     Prec(2)=Prec(1);
    ...
    stParameters.Q_TABLE_1;
    stParameters.LocUAV;
    stParameters.LocUAV_1;
    stParameters.Action;
    figure(1);
    hold on;
